...
# 25. Nova proposta estrutural: áreas navegáveis dentro da ilha dinâmica

## 25.1. Motivo da mudança

O problema de responsividade vertical persiste porque a ilha dinâmica aberta continua tentando exibir, no mesmo fluxo vertical:
...
Mesmo reduzindo espaçamentos e aplicando rolagem interna, a interface continua ficando muito alta. Em telas menores, a área de notificações perde espaço até praticamente desaparecer.

Nesse cenário, continuar comprimindo os componentes não resolve a causa estrutural.

A melhor alternativa é separar a ilha aberta em **áreas navegáveis**, cada uma com responsabilidade própria.

---

## 25.2. Proposta de arquitetura

A ilha dinâmica aberta pode ser dividida em três páginas internas:
...
- estado tocando ou pausado;
- ação para abrir o aplicativo de origem;
- sem barra de progresso enquanto ela não for confiável.
...
- luz noturna;
- não perturbe;
- ações do sistema separadas visualmente.

### Página 3 — Notificações

- título completo “Notificações”;
- botão completo “Limpar”;
- lista rolável;
- agrupamento por aplicativo, caso necessário;
- altura útil muito maior que no layout atual.

Essa separação resolve diretamente a disputa vertical entre player e notificações.

---

# 26. Navegação por arrastar horizontalmente

## 26.1. Interação principal

O usuário pode pressionar o mouse e arrastar para o lado:

- arrastar para a esquerda abre a próxima área;
- arrastar para a direita volta para a área anterior.

No touch:

- swipe horizontal;
- gesto natural, semelhante a páginas de carrossel.

A navegação também deve funcionar por:

- clique em indicadores;
- setas do teclado;
- `PageUp` e `PageDown`, opcionalmente;
- botões discretos de navegação, caso necessário para descoberta.

---

## 26.2. Estrutura visual sugerida
...
- página ativa: ponto preenchido;
- páginas inativas: pontos discretos;
- devem ser clicáveis;
- precisam de rótulos acessíveis.

Uma alternativa ainda mais clara:
...
Porém, abas textuais ocupam mais largura. Para a ilha dinâmica, pontos ou ícones são mais compactos.

---

# 27. Vantagens dessa abordagem

## 27.1. Resolve a disputa por altura

Cada página usa quase toda a área útil.

O player deixa de empurrar a lista de notificações para fora da tela.

---

## 27.2. Melhora a hierarquia
...
## 27.3. Reduz densidade visual

O usuário não precisa processar todos os recursos ao mesmo tempo.

---

## 27.4. Permite responsividade real

Em telas menores:

- a largura continua adaptável;
- a altura fica controlada;
- cada página possui seu próprio scroll apenas quando necessário.

---

## 27.5. Facilita futuras extensões

Novas áreas podem ser adicionadas sem empilhar mais conteúdo verticalmente.
...
Isso só deve ser feito com moderação para não transformar a ilha em um sistema completo de widgets.

---

# 28. Cuidados importantes na navegação por arrastar

## 28.1. Não confundir drag horizontal com sliders

A página de controles possui sliders horizontais.

Esse é um conflito direto:

- arrastar o slider deve alterar volume ou brilho;
- arrastar a área deve trocar de página.

### Regra obrigatória

Quando o gesto começar sobre:
...
a navegação entre páginas não deve ser iniciada.

Exemplo:

```js
const interactiveSelector =
  'button, input, [role="slider"], a, textarea, select, [data-no-swipe]';
...
## 28.2. Usar threshold mínimo

A página não deve mudar por qualquer movimento pequeno do mouse.

Recomendado:

- distância mínima: `48–64 px`;
- velocidade mínima opcional;
- troca apenas se o gesto tiver intenção horizontal clara.
...
const passedThreshold = Math.abs(deltaX) > 56;
```

---

## 28.3. Evitar troca acidental durante scroll vertical

Na página de notificações, o usuário precisa rolar verticalmente.

A navegação horizontal só deve ocorrer quando:

```text
movimento horizontal > movimento vertical
```

Caso contrário, a lista deve rolar normalmente.

---

## 28.4. Usar Pointer Events
...
Evitar implementar sistemas separados para mouse e touch quando não for necessário.

---

## 28.5. Captura de ponteiro
...
Isso evita perder o gesto quando o cursor sai temporariamente do componente.

---

# 29. Comportamento de animação

## 29.1. Transição recomendada
...
## 29.2. Durante o arraste

Durante o drag, a transição deve ser temporariamente desativada:

```css
.pages-track[data-dragging="true"] {
  transition: none;
}
```

A posição deve acompanhar o cursor.

Ao soltar:

- completar a troca se passar o threshold;
- voltar para a página atual se não passar.

---

## 29.3. Respeitar redução de movimento
...
# 30. Persistência da página atual

A ilha deve lembrar a última área aberta durante a sessão.

Exemplo:

- usuário abre Notificações;
- fecha a ilha;
- reabre logo depois;
- continua em Notificações.

Porém, eventos contextuais podem alterar isso:

- ao iniciar mídia, abrir em Mídia;
- ao receber notificação importante, exibir preview sem trocar permanentemente a página;
- ao abrir manualmente pelo usuário, restaurar a última página visitada.

---

# 31. Sugestão de fluxo por contexto

## 31.1. Quando não há mídia
...
## 31.3. Quando há novas notificações

Não trocar a página automaticamente de forma agressiva.

Melhor comportamento:

- exibir badge no indicador de Notificações;
- mostrar uma preview temporária;
- deixar o usuário navegar quando desejar.

Trocas automáticas de página podem interromper uma interação em andamento.

---

# 32. Estado responsivo de cada página

## 32.1. Página de mídia
...
Sem progresso bugado.

---

## 32.2. Página de controles

Pode ter rolagem vertical apenas se estritamente necessário:
...
Porém, o objetivo deve ser caber sem scroll na maioria das alturas.

---

## 32.3. Página de notificações
...
# 33. Correção definitiva dos títulos truncados
...
não devem existir nessa interface.

Esse problema persiste porque a implementação continua aceitando truncamento como solução para labels curtas.

---

## 33.1. Substituições obrigatórias

```text
Quick Contr...  → Controles rápidos
Notificatio...  → Notificações
Limp...         → Limpar
...
## 33.2. Labels curtas nunca devem usar ellipsis
...
O container deve acomodar o título.

---

## 33.3. Cabeçalho de seção correto
...
## 33.4. Não usar largura fixa no título
...
## 33.5. Por que separar por páginas ajuda

Na arquitetura atual, vários títulos disputam o mesmo espaço vertical e horizontal com outros elementos.

Com páginas separadas:

- a página de Controles pode exibir “Controles rápidos” por completo;
- a página de Notificações pode exibir “Notificações” e “Limpar” sem compressão;
- não é necessário abreviar nenhum desses textos.

---

# 34. Estrutura recomendada completa

```html
<aside class="dynamic-island-panel">
  <header class="panel-header">
    ...
  </header>

  <nav class="page-indicators" aria-label="Áreas da ilha dinâmica">
    <button aria-label="Mídia" aria-current="page"></button>
    <button aria-label="Controles"></button>
    <button aria-label="Notificações"></button>
  </nav>

  <div class="pages-viewport">
    <div class="pages-track">
      <section class="panel-page media-page">
        ...
      </section>

      <section class="panel-page controls-page">
        ...
      </section>

      <section class="panel-page notifications-page">
        ...
      </section>
    </div>
  </div>
...
# 35. Acessibilidade da navegação

A navegação não pode depender exclusivamente de arrastar.
...
- `aria-current`;
- labels acessíveis;
- anúncio da página atual para leitores de tela.

Exemplo:

```html
<div aria-live="polite" class="sr-only">
  Página Controles, 2 de 3
</div>
```

---

# 36. Critérios de aceite para essa nova arquitetura

- [ ] player não empurra notificações para fora;
- [ ] cada área ocupa 100% da largura útil;
- [ ] é possível navegar com mouse drag;
- [ ] é possível navegar com touch swipe;
- [ ] é possível navegar por clique;
- [ ] é possível navegar com teclado;
- [ ] sliders não ativam swipe;
- [ ] scroll vertical de notificações não troca de página;
- [ ] títulos aparecem completos;
- [ ] “Limpar” aparece completo;
- [ ] idioma está unificado;
- [ ] a página atual possui indicador visível;
- [ ] o painel respeita a altura da viewport;
- [ ] nenhuma notificação é recortada;
- [ ] o estado da página não muda sozinho durante interação;
- [ ] `prefers-reduced-motion` é respeitado.

---

# 37. Recomendação final desta iteração

A proposta de separar o painel em áreas navegáveis é mais adequada do que continuar tentando encaixar todos os componentes em uma única coluna vertical.

Ela resolve simultaneamente:

- disputa por altura;
- recorte das notificações;
- excesso de densidade;
- agrupamento semântico;
- dificuldade de responsividade;
- títulos truncados por falta de espaço estrutural.
...
> O gesto de navegação nunca pode competir com sliders, rolagem vertical ou outros controles interativos.
